...
export const banUser = asyncHandler(async (req: Request, res: Response) => {
  ...
  return ApiResponse.ok(await svc.banUser(req.user!.userId, p(req, 'userId'), note)).send(res);
});

export const unbanUser = asyncHandler(async (req: Request, res: Response) => {
  const { note } = z.object({ note: z.string().optional() }).parse(req.body);
  return ApiResponse.ok(await svc.unbanUser(req.user!.userId, p(req, 'userId'), note)).send(res);
});
...
export const changeRole = asyncHandler(async (req: Request, res: Response) => {
  ...
  return ApiResponse.ok(await svc.changeUserRole(req.user!.userId, p(req, 'userId'), role)).send(res);
});
